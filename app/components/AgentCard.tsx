...
// Generate card metadata from agent data if none stored
function generateCardMetadata(props: AgentCardProps): CardMetadata {
  const { agentType, tier, reputation, battlesWon, battlesLost, projectsCompleted, capabilities, name, tokenId } = props
  ...
  // Use deterministic art colors for the card palette
  const art = generateCardArt(tokenId, name)

  const tierPalettes: Record<string, { edition: string; rarity: string }> = {
    BRONZE:   { edition: 'Base Edition · 2025', rarity: '● COMMON' },
    SILVER:   { edition: 'Genesis Edition · 2025', rarity: '◆ UNCOMMON' },
    GOLD:     { edition: 'Shadow Edition · 2025', rarity: '★ RARE' },
    PLATINUM: { edition: 'Inferno Edition · 2025', rarity: '◆ ULTRA RARE' },
    DIAMOND:  { edition: 'Apex Edition · 2025', rarity: '★ LEGENDARY' },
  }
  const tp = tierPalettes[tier?.toUpperCase()] || tierPalettes.BRONZE
...
export default function AgentCard(props: AgentCardProps) {
  ...
  return (
    <div onClick={handleClick} className="select-none" style={{ perspective: '1000px', cursor: 'pointer' }}>
      <div style={{
        width: 320, height: 560, position: 'relative',
        ...
        <div style={{
          ...
          borderRadius: 20, padding: 3, background: meta.borderGrad,
        }}>
          <div style={{
            ...
            <div style={{ padding: '14px 16px 8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontFamily: 'Orbitron, monospace', fontSize: 15, fontWeight: 900, color: '#fff', letterSpacing: 1 }}>{props.name}</div>
                <div style={{ fontSize: 10, color: '#64748b', letterSpacing: 1, fontWeight: 600, marginTop: 2 }}>BATTLE AGENT</div>
              </div>
            ...
            <div style={{ display: 'flex', gap: 5, padding: '0 16px 10px' }}>
              {elements.map(el => {
                ...
                return <span key={el} style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', padding: '3px 10px', borderRadius: 20, background: ec.bg, color: ec.text, border: `1px solid ${ec.border}` }}>{el}</span>
              })}
            </div>
            ...
            {/* Flavor text */}
            <div style={{ padding: '8px 16px 4px', fontSize: 10, color: '#475569', fontStyle: 'italic', lineHeight: 1.4 }}>{meta.flavor}</div>
              ...
              {meta.stats.map(s => {
                ...
                return (
                  <div key={s.n} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: '#64748b', width: 32, flexShrink: 0 }}>{s.n}</span>
                    <div style={{ flex: 1, height: 8, background: 'rgba(255,255,255,0.08)', borderRadius: 4, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${pct}%`, background: s.c, borderRadius: 4 }} />
                    </div>
                    <span style={{ fontFamily: 'Orbitron, monospace', fontSize: 10, fontWeight: 700, color: '#e2e8f0', width: 28, textAlign: 'right' }}>{s.v}</span>
                  </div>
                )
              })}
            </div>
            ...
            <div style={{ padding: '8px 16px 12px', borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: 6 }}>
              <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: '#475569', marginBottom: 6 }}>Abilities</div>
              {meta.moves.map((m, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6, padding: '5px 8px', borderRadius: 8 }}>
                  <div style={{ width: 18, height: 18, borderRadius: '50%', background: `${m.color}22`, border: `1px solid ${m.color}55`, marginRight: 8, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 3, marginRight: 8 }}>
                    {Array.from({ length: m.energy }, (_, j) => (
                      <div key={j} style={{ width: 10, height: 10, borderRadius: '50%', background: m.color, border: '1px solid rgba(255,255,255,0.3)' }} />
                    ))}
                  </div>
                  <span style={{ fontFamily: 'Orbitron, monospace', fontSize: 11, fontWeight: 700, color: '#fbbf24' }}>{m.dmg > 0 ? m.dmg : '—'}</span>
                </div>
              ))}
            </div>

            {/* Footer */}
            <div style={{ padding: '8px 16px 12px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: 9, color: '#475569', letterSpacing: 1, textTransform: 'uppercase', fontWeight: 600 }}>{meta.edition}</div>
        ...
        <div style={{
          position: 'absolute', inset: 0, backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden',
          borderRadius: 20, padding: 3, background: meta.borderGrad, transform: 'rotateY(180deg)',
        }}>
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
              <div>
                ...
                <div style={{ fontSize: 9, color: '#475569', letterSpacing: 1.5, fontWeight: 600, marginTop: 2 }}>AGENT DOSSIER</div>
              </div>
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                {elements.map(el => {
                  const ec = ELEMENT_COLORS[el]
                  return <span key={el} style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', padding: '2px 8px', borderRadius: 20, background: ec.bg, color: ec.text, border: `1px solid ${ec.border}` }}>{el}</span>
                })}
              </div>
            </div>
            ...
            <div style={{ margin: '12px 0 4px', display: 'flex', justifyContent: 'center' }}>
              <div style={{ width: 70, height: 70, borderRadius: '50%', border: '2px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {meta.abilities.map((ab, i) => (
              <div key={i} style={{ background: 'rgba(255,255,255,0.04)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '10px 12px', marginBottom: 8 }}>
                <div style={{ fontFamily: 'Orbitron, monospace', fontSize: 11, fontWeight: 700, color: '#e2e8f0', marginBottom: 4 }}>{ab.name}</div>
                <div style={{ fontSize: 11, color: '#64748b', lineHeight: 1.5 }}>{ab.desc}</div>
              </div>
            ))}
            ...
            {meta.traits.map((t, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
                <span style={{ fontSize: 11, color: '#64748b', fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase' }}>{t.k}</span>
                <span style={{ fontFamily: 'Orbitron, monospace', fontSize: 11, fontWeight: 700, color: '#e2e8f0' }}>{t.v}</span>
              </div>
            ))}
